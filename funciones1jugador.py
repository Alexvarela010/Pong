...
def manejar_colisiones_bloques1ju(bloques, VELOCIDAD_PELOTA, cuadrados_extra):
    for c, columna in enumerate(bloques[:] ):
        for bloque in columna[:]:
            if PELOTA.colliderect(bloque):
                ...
                elif VELOCIDAD_PELOTA[1] > 0:  # La pelota se mueve hacia abajo
                    direccion = [0, -4]  # El cuadrado se moverá hacia arriba
                else:  # La pelota se mueve hacia arriba
                    direccion = [0, 4]  # El cuadrado se moverá hacia abajo
...
def mover_raquetas1ju():
    ...
    # Si la pelota está más arriba, la raqueta se mueve hacia arriba
    if PELOTA.centery < RAQUETA2.centery and RAQUETA2.top > 0:
        RAQUETA2.y -= VELOCIDAD_RAQUETA // 2  # Velocidad reducida para hacerlo más manejable
    # Si la pelota está más abajo, la raqueta se mueve hacia abajo
    elif PELOTA.centery > RAQUETA2.centery and RAQUETA2.bottom < ALTO:
        RAQUETA2.y += VELOCIDAD_RAQUETA // 2

    # Añadir un poco de aleatoriedad
    if random.randint(0, 10) > 7:  # Probabilidad de un 30% de que se movima de manera aleatoria
        RAQUETA2.y += random.choice([-5, 5])
